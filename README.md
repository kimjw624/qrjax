...
JAX preallocates 75% of VRAM by default. On a laptop GPU:
...
```bash
alias qr='cd ~/qrjax && source .venv/bin/activate && export XLA_PYTHON_CLIENT_MEM_FRACTION=.92'
```

## Quick start — the best configuration

PID baseline with a residual, 200 Hz, thrust-channel low-pass filter. About
20 minutes on an RTX 4050.
...
Paired comparison of all four arms — PD, PID, PD+residual, PID+residual — on
one frozen disturbance bank. Every arm sees identical episodes, and the script
aborts if the draws diverge.
...
Full figure and table set for a paper:
...
That writes `paper_figures/` with 3-D trajectories, error-vs-time, actuator
traces and summary plots, every figure labelled with its uncertainty case
(`k`, `|F|`, `kf`, `arm`, `g`, and whether `g` is inside the stability margin).
